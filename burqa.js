...
_clickMap: {
	'l_alt_link':_              => $TOP.LOG ? (location.href='//the.burqa.com') : $DAT.toggleStage($TOP.ON),
...
EVT: {
	...
	setup: () => {
		...
		if (nav && nav.type === "reload")
			location.href = 'https://the.burqa.com';
...
GUI: {
	...
	menuClick: sub => {
		if($I(['www','top'],!$TOP.LOG&&$DAT.MODE!='bid'?sub:null) >= 0) {
			...
			const url=`//${sub=='www'?'':sub+'.'}${$M(/(stage\.)?[^.]+\.[A-Z0-9]+$/i,$D.domain)?_M[0]:'the.burqa.com'}`, win=$W.open(url,sub);
			if(win) win.focus();
...
($DAT.FETCHING=$E('l_logo').classList.remove('l_net_loading'))
